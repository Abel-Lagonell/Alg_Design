...
def makeCoverageGraph(randCov:list[int], greeCov:list[int], coveCov:list[int], ran:list[int], title:str, x_axis:str):
    fig = go.Figure()
...
def makeBudVSCovBudgetGraph(randCov:list[int], greeCov:list[int], coveCov:list[int], randBud:list[int], greeBud:list[int], coveBud:list[int], ran:list[int], title:str, x_axis:str):
    fig = go.Figure()
    ...
    m, b = np.polyfit(randBud, randCovBud, 1)
    line = m * np.array(randBud) + b
    fig.add_trace(go.Scatter(x=ran, y=line, mode='lines', name='Random Trend', marker=dict(color='blue')))
    ...
    m, b = np.polyfit(greeBud, greeCovBud, 1)
    line = m * np.array(greeBud) + b
    fig.add_trace(go.Scatter(x=ran, y=line, mode='lines', name='Greedy Trend', marker=dict(color='red')))
    ...
    m, b = np.polyfit(coveBud, coveCovBud, 1)
    line = m * np.array(coveBud) + b
    fig.add_trace(go.Scatter(x=ran, y=line, mode='lines', name='Greedy Set Cover Trend', marker=dict(color='green')))
    fig.show()

#Getting the data for the graph
def getData(type:bool, ran:list[int] ,graph:bool, randCov:list[int], randBud:list[int] , greeCov:list[int], greeBud:list[int], coveCov:list[int], coveBud:list[int]): 
...
for independent in [0,1]:
    for constant in [0,1]:
        ...
        getData(type= type, graph=constant, randCov= changing_RAND, greeCov= changing_GREED, coveCov= changing_COVER, ran=list, randBud= changing_RAND_BUDGET, greeBud= changing_GREED_BUDGET, coveBud= changing_COVER_BUDGET)
        makeCoverageGraph(randCov=changing_RAND, greeCov=changing_GREED, coveCov=changing_COVER, title=title, x_axis=x_axis, ran=list)
...
for independent in [0,1]:
    for constant in [0,1]:
        ...
        getData(type= type, graph=constant, randCov= changing_RAND, greeCov= changing_GREED, coveCov= changing_COVER, ran=list, randBud= changing_RAND_BUDGET, greeBud= changing_GREED_BUDGET, coveBud= changing_COVER_BUDGET)
        makeBudVSCovBudgetGraph(randCov=changing_RAND, greeCov=changing_GREED, coveCov=changing_COVER, randBud=changing_RAND_BUDGET, greeBud=changing_GREED_BUDGET, coveBud=changing_COVER_BUDGET, ran=list, title=title, x_axis=x_axis)